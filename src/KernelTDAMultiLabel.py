...
def standardGraphFile(dataset, file, datapath, h_filt, iter, filtration,max_allowed_filtration):
    ...
    activation_discovery(dataset, edges_asdf, graphindicator_aslist, id_max, id_min, progress, total_degree,
    ...
    if(max_activation-min_activation)>max_allowed_filtration:
        max_activation=int(np.percentile(id_max, 90))
    ...
    for graphid in unique_graphindicator:
        kernelize_graph(diag_matrix, edges_asdf, filtr_range, filtration, graphid, graphindicator_aslist, iter,
...
def kernelize_graph(diag_matrix, edges_asdf, filtr_range, filtration, graphid, graphindicator_aslist, iter, progress,
                    random_dict):
...
def activation_discovery(dataset, edges_asdf, graphindicator_aslist, id_max, id_min, progress, total_degree,
                         unique_graphindicator):
...
if __name__ == '__main__':
    ...
    datasets = ("REDDIT-MULTI-5K","REDDIT-MULTI-12K","FIRSTMM_DB",'BZR','MUTAG', 'DD', 'PROTEINS', 'DHFR', 'NCI1', 'COX2')
