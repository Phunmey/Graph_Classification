...
def standardGraphFile(dataset, data_path):
    ...
    for i in unique_graph_indicator:
        ...
        motifs_count = train_graph.motifs_randesu(size=3)  # obtain motif count
    ...
    df = pd.DataFrame(motifs, columns=['motif1', 'motif2', 'motif3', 'motif4'])
    df.insert(0, 'dataset', dataset)

    res1 = df.groupby('dataset')[['motif3', 'motif4']].agg(['std','mean'], axis=0)
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        df1.append(func)
    df2 = pd.concat(df1)
    #res1 = df2.groupby('dataset')[['motif3', 'motif4']].mean()
    #print(res1)

    df2.to_csv("C:/Code/src/Graph_statistics/motif3.csv")

#, 'MUTAG', 'PROTEINS', 'DHFR', 'NCI1', 'COX2', 'REDDIT-MULTI-5K', 'REDDIT-MULTI-12K'
