...
def standardGraphFile(dataset, file, data_path, thresh):
    ...
    for w in test_set:
        ...
        testnorm_dist_matrix = test_dist_matrix / np.nanmax(test_dist_matrix)  # normalized distancematrix
        # test_dim_reduction = MDS(n_components=3, dissimilarity='precomputed').fit_transform(testnorm_dist_matrix)
        test_alpha_complex = gd.AlphaComplex(points=testnorm_dist_matrix)  # initialize alpha complex
