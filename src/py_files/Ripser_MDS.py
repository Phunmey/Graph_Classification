...
def standardGraphFile(dataset, file, data_path, iter, step):
    ...
    for i in train_set:
        ...
        train_graph_edges = df_edges[df_edges['from'].isin(train_id_loc)] #obtain the edges with source node as train_graph_id
        train_graph = Graph.TupleList(train_graph_edges.itertuples(index=False), directed=False, weights=True) #obtain the graph
        train_dist_matrix = np.asarray(Graph.shortest_paths_dijkstra(train_graph)) #obtain the distance matrix from graph
        train_dist_matrix[train_dist_matrix == inf] = 0 #replace inf with zero
        embedding = MDS(n_components=3, dissimilarity='precomputed').fit_transform(train_dist_matrix) #dimension reduction
        norm_dist_matrix = embedding / np.nanmax(embedding) #normalized reduced matrix
        train_diagrams = ripser(norm_dist_matrix, thresh=0.5, maxdim=1, distance_matrix=False)['dgms'] #run vietoris-rips filtration
    ...
    for w in test_set:
        ...
        test_embedding = MDS(n_components=3, dissimilarity='precomputed').fit_transform(test_dist_matrix)
...
if __name__ == '__main__':
    ...
    outputFile = "/home/taiwo/projects/def-cakcora/taiwo/results/" + 'MDSResults.csv'
