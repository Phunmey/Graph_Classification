...
def standardGraphFile(dataset, file, data_path, kernel_iter):
    start = time()
    ...
    for i in unique_graph_indicator:
        ...
        edges_loc = df_edges[df_edges['from'].isin(graphid_loc)] #obtain the edges needed
        edges_loc_asset = (edges_loc.to_records(index=False)).tolist() #obtain edges location
        nodes_aslist = ((edges_loc['from'].append(edges_loc['to'])).unique()).tolist() #obtain unique edges
        ext = {k: nodes_dict[k] for k in nodes_aslist if k in nodes_dict} #obtain nodelabels as a dictionary
        edges_nodes = [edges_loc_asset, ext] #list of edges and nodelabels
    ...
    WL = WeisfeilerLehmanOptimalAssignment(n_iter=kernel_iter, normalize=True)
...
# RandomForest hyper-parameters tuning
    ...
    file.write(dataset + "\t" + str(time_taken) + "\t" + str(t3 - t2) +
               ...
               "\t" + str(kernel_iter) + "\t" + str(flat_conf_mat) + "\n")
    file.flush()
...
if __name__ == '__main__':
    ...
    outputFile = "/home/taiwo/projects/def-cakcora/taiwo/New_Results/" + 'WLOAKernelresults.csv'
    file = open(outputFile, 'w')
    for dataset in datasets:
        for iter_ in (2, 3, 4):
            for duplication in np.arange(5):
                standardGraphFile(dataset, file, data_path, kernel_iter=iter_)
    file.close()