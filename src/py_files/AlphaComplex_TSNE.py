...
def standardGraphFile(dataset, file, data_path, thresh):
    ...
    for i in train_set:
        ...
        train_dim_reduction = TSNE(n_components=3, init='pca', learning_rate='auto').fit_transform(norm_dist_matrix)
        ...
        train_persist_2 = train_diagrams[:, 1][np.where(train_diagrams[:, 0] == 2)]

        # obtain betti numbers for the unique dimensions
        threshold = np.arange(0, thresh + 0.05, 0.1)
        train_betti_0 = []
        train_betti_1 = []
        for j in threshold:
            b_0 = 0
    ...
    for w in test_set:
        ...
        test_dim_reduction = TSNE(n_components=3, init='pca', learning_rate='auto').fit_transform(testnorm_dist_matrix)
        ...
        test_diagrams = np.asarray(test_simplex_tree.persistence(2),
                                   dtype='object')  # run AlphaComplex filtration on the normalized distance matrix
        # gd.plot_persistence_diagram(diagrams)
        # plt.show()

        # splitting the dimension into 0 and 1
        test_persisted_0 = test_diagrams[:, 1][np.where(test_diagrams[:, 0] == 0)]
        test_persisted_1 = test_diagrams[:, 1][np.where(test_diagrams[:, 0] == 1)]
        test_persisted_2 = test_diagrams[:, 1][np.where(test_diagrams[:, 0] == 2)]
    ...
    t3 = time()
    print(f'AlphaComplex took {time_taken} seconds, training took {t3 - t2} seconds')
    flat_conf_mat = (str(conf_mat.flatten(order='C')))[1:-1]
    file.write(dataset + "\t" + str(time_taken) + "\t" + str(t3 - t2) +
               "\t" + str(accuracy) + "\t" + str(auc) + "\t" + str(thresh) + "\t" + str(flat_conf_mat) + "\n")
    file.flush()
...
if __name__ == '__main__':
    ...
    outputFile = "C:/Code/New_Results/" + 'AlphaTSNEresult.csv'
