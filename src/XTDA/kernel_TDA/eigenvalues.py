...
def standardGraphFile(dataset, datapath):
    ...
    return unique_graphindicator, graphlabels_aslist, random_dict, graphindicator_aslist, edges_asdf


def activation_discovery(dataset, edges_asdf, graphindicator_aslist, unique_graphindicator):
    ...
    plt.title(dataset)
    # plt.show()
    plt.savefig("../../../results/figures/" + dataset + "DegreeStats.png")
...
def kernelize_graph(unique_graphindicator, edges_asdf, filtr_range, filtration, dataset, graphindicator_aslist, iter, progress, random_dict, graphlabels_aslist):
    start4 = time()
    feature_matrix = []
...
def train_test_rf(Param_Grid, dataset, g_test, g_train, num_cv, y_test, y_train, time_taken, file, h_filt, iter):
    # start training
...
def main(file, h_filt, iter, filtration, max_allowed_filtration):
    unique_graphindicator, graphlabels_aslist, random_dict, graphindicator_aslist, edges_asdf = standardGraphFile(dataset, datapath)
    max_activation, min_activation, progress = activation_discovery(dataset, edges_asdf, graphindicator_aslist, unique_graphindicator)
    filtr_range, filtration, h_filt = filtration_discovery(dataset, filtration, h_filt, max_activation,max_allowed_filtration, min_activation)
    g_train, g_test, y_train, y_test, time_taken, t4 = kernelize_graph(unique_graphindicator, edges_asdf, filtr_range,filtration, dataset, graphindicator_aslist, iter, progress, random_dict,graphlabels_aslist)
    Param_Grid, num_cv = rf_preprocess()
    train_test_rf(Param_Grid, dataset, g_test, g_train, num_cv, y_test, y_train, time_taken, file, h_filt, iter)
            ...
            for half in (True, False):
                for iter_ in (2, 3, 4):
                    for duplication in range(5):
                        main(output_file, h_filt=half, iter=iter_, filtration=filtr_type, max_allowed_filtration=100)
