...
def standardGraphFile(dataset, file, datapath, h_filt, kernel_iter, filtration, max_allowed_filtration):
    start = time()
    ...
    for graphid in unique_graphindicator:
        kernelize_graph(feature_matrix, edges_asdf, filtr_range, filtration, graphid, graphindicator_aslist, kernel_iter,
    ...
    file.write(dataset + "\t" + str(time_taken) + "\t" + str(t3 - t2) +
               "\t" + str(accuracy) + "\t" + str(auc) + "\t" + str(kernel_iter) + "\t" + str(h_filt) + "\t" +
               str(flat_conf_mat) + "\n")
    file.flush()
...
def activation_discovery(dataset, edges_asdf, graphindicator_aslist, node_degree_max, node_degree_min, progress,
    ...
    for graphid1 in unique_graphindicator:
        ...
        edges_loc1 = edges_asdf[edges_asdf['from'].isin(graphid_loc1)]  # obtain edges that corresponds to these locations
...
def kernelize_graph(feature_matrix, edges_asdf, filtr_range, filtration, graphid, graphindicator_aslist, kernel_iter, progress,
                    random_dict):
    ...
    wl = WeisfeilerLehman(n_iter=kernel_iter, base_graph_kernel=VertexHistogram, normalize=True)
    ...
    feature_matrix.append(compute_mean)
...
if __name__ == '__main__':
    datapath = "/home/taiwo/projects/def-cakcora/taiwo/data"  # dataset path on computer such as  "C:/data"
    datasets = ('REDDIT-MULTI-12K', "COX2")
    outputFile = "/home/taiwo/projects/def-cakcora/taiwo/New_Results/" + 'Eigenvectorresults.csv'
    output_file = open(outputFile, 'w')
    for dataset_name in datasets:
        for iter_ in (2, 3, 4):
            for duplication in range(5):
                standardGraphFile(dataset_name, output_file, datapath, h_filt=False, kernel_iter=iter_,
