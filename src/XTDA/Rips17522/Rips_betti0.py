...
def read_csv(dataset):
    ...
    return X_train, X_test, y_train, y_test, graph_indicators, df_edges, graph_labels, unique_graph_indicator

def plot_max( unique_graph_indicator, graph_indicators, df_edges):
    max_num = []
    retrieve_i = []
    for i in  unique_graph_indicator:
        ...
        distance_matrix = np.asarray(Graph.shortest_paths_dijkstra(create_graph))
        dist_max = np.nanmax(distance_matrix[distance_matrix != np.inf])
        max_num.append(dist_max)
        retrieve_i.append(i)

    # print('The global maximum for this graph is:' + str(np.max(max_num)))
    # plt.hist(max_num, color='b')
    # plt.xlabel('max_numbers')
    # plt.ylabel('graph_indicator')  # obtained by dividing the node count of the filtration by the data node count
    # plt.title('graph_maximums')
    # plt.show()
    #plt.savefig("../../results/" + dataset + "DegreeStats.png")

    return max_num

def ripser_train(X_train, X_test, thresh, graph_indicators, df_edges, step_size):  #this is for the train test
    start2 = time()
    train_betti =[]
    for i in X_train:
        graph_id = i
        ...
        #edge_bet =  create_traingraph.edge_betweenness()
    ...
    for j in X_test:
        ...
        id_location = [index for index, element in enumerate(graph_indicators) if element == graph_id] #list the index of the graph_id locations
        graph_edges = df_edges[df_edges['from'].isin(id_location)]
        create_testgraph = Graph.TupleList(graph_edges.itertuples(index=False), directed=False, weights=False)
...
def random_forest(dataset, Param_Grid, train_data, test_data, y_train, y_test, ripser_time, thresh, num_cv, step_size):
...
def main():
    X_train, X_test, y_train, y_test, graph_indicators, df_edges, graph_labels, unique_graph_indicator = read_csv(dataset)
    max_num = plot_max( unique_graph_indicator, graph_indicators, df_edges)
    train_data, test_data, ripser_time = ripser_train(X_train, X_test, thresh, graph_indicators, df_edges, step_size)
    Param_Grid, tuning_time, num_cv = tuning_hyperparameter()
    random_forest(dataset, Param_Grid, train_data, test_data, y_train, y_test, ripser_time, thresh, num_cv, step_size)
...
if __name__ == '__main__':
    ...
    outputFile = "../../../results/" + 'Rips_Betti0.csv'
    file = open(outputFile, 'w')
    for dataset in data_list:
        for thresh in [1]:
            for step_size in [10,20,50]: #we will consider step sizes [3,5,10,20,50] for epsilon
