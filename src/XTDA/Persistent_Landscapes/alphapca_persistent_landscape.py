...
def landscape_train(X_train, thresh, graph_indicators, df_edges, step_size):  # this is for the train test
   ...
   # train_silhouette = []
    for i in X_train:
        ...
        merge_array = [train_persist_0, train_persist_1]
    ...
    return train_landscape, train_time, sample_range


def landscape_test(X_test, graph_indicators, df_edges, sample_range, train_time, train_landscape):
    start3 = time()
    test_landscape = []
   # test_silhouette = []
    for j in X_test:
        ...
        merged_array = [test_persist_0, test_persist_1]
...
def random_forest(dataset, Param_Grid, train_data, test_data, y_train, y_test, landscape_time, thresh, num_cv,
    ...
    print(f'Landscape took {landscape_time} seconds, training took {training_time} seconds')
...
def main():
    ...
    train_landscape, train_time, sample_range = landscape_train(X_train, thresh, graph_indicators, df_edges, step_size)
    train_data, test_data, landscape_time = landscape_test(X_test, graph_indicators, df_edges, sample_range, train_time, train_landscape)
...
if __name__ == '__main__':
    ...
    outputFile = "../../../results/" + 'alphapcapersistent.csv'
