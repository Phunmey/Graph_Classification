...
for hidx in range(len(homdim)):
    ...
    if PV_type == 'PI':
        ...
        im_bnds = [np.quantile(vpdtr[:, 0], 0.), np.quantile(vpdtr[:, 0], 1.),
                   np.quantile(vpdtr[:, 1] - vpdtr[:, 0], 0.), np.quantile(vpdtr[:, 1] - vpdtr[:, 0], 1.)]
        print(im_bnds)
        PV_params = {'bandwidth': sigma, 'weight': lambda x: const_fct * np.tanh(x[1]) ** power_fct,
...
pck.dump({'hdims': homdim, 'ts_train': ts_train, 'ts_test': ts_test, 'data_train': data_train, 'data_test': data_test,
          'PD_train': PD_train, 'PD_test': PD_test, 'PV_train': PVs_train, 'PV_test': PVs_test,
          'label_train': label_train, 'label_test': label_test, 'PV_params': PVs_params, 'PV_type': PV_type},
         open(dataset_name + suffix_name + ".pkl", 'wb'))
